...
## CONTEST08-1002 Breaking Down News

### Description

给定 $$ L, R, a_1 \ldots a_n$$, 要求把序列划分成若干段，每段长度在 $$ [L, R] $$ 区间内。每一段的价值为 $$v_k = \operatorname{sgn} (\sum_{i=l_k}^{r_k} a_i) $$，最大化 $$v_k$$ 的和。

$$\sum n\le 9\times 10^6$$

### Solution

首先有一个比较显然的 $$O(n\log n)$$ 算法，对前缀和建立权值线段树，每次 $$i\to i+1$$ 时，需要在某个位置删除某个值，插入一个值。可以在线段树的每个叶子维护一个 set 实现，这样转移时只要分成 $$f_k+1,f_k+0, f_k-1$$ 三个情况分别询问一下线段树即可。

注意到只有 $$f_k+1,f_k+0, f_k-1$$ 三种情况，若 $$[i-r,i-l]$$ 中的 $$f$$ 最大值为 $$x$$，那么 $$f_i \ge x-1$$，那么只有 $$f_k\ge x-1$$ 是有用的，其余不可能变得更好。我们为了让 $$f_i$$ 取到 $$x+1$$ 或 $$x-1+1$$，需要维护 $$f_k = x, x-1$$ 对应的前缀和的最小值和最大值，用单调队列可以快速的求 $$x$$，同样的，对每个权值维护一个前缀和的单调队列即可转移，时间复杂度 $$O(n)$$。

## CONTEST08-1007 Gaming of Co-prime Disallowance
